@extends('layout.master')
@section('title', 'Visa Applications')
@section('header-title', 'Visa Applications')
@section('content')
    <div class="content-page">
        <div class="content">
            <div class="container">
                <div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column">
                    <div class="flex-grow-1">
                        <h4 class="fs-18 fw-semibold m-0">
                            Visa Applications
                        </h4>
                    </div>
                </div>
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h5 class="mb-0">
                                    Visa Applications List
                                </h5>
                            </div>
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="datatable" class="table table-bordered dt-responsive nowrap align-middle">
                                        <thead class="table-light">
                                            <tr>
                                                <th>S:NO</th>
                                                <th>Applicant Name</th>
                                                <th>Phone</th>
                                                <th>Country</th>
                                                <th>Visa Type</th>
                                                <th>B2B Rate</th>
                                                <th>Visa Fee</th>
                                                <th>Status</th>
                                                {{-- <th>Action</th> --}}
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @foreach ($applications as $application)
                                                <tr>
                                                    <td>
                                                        {{ $loop->iteration }}
                                                    </td>
                                                    <td>
                                                        <strong>
                                                            {{ $application->full_name ?? 'N/A' }}
                                                        </strong>
                                                    </td>
                                                    <td>
                                                        {{ $application->phone ?? 'N/A' }}
                                                    </td>
                                                    <td>
                                                        {{ $application->visaType->country->country_name ?? 'N/A' }}
                                                    </td>
                                                    <td>
                                                        {{ $application->visaType->visa_name ?? 'N/A' }}
                                                    </td>
                                                    <td>
                                                        {{ $application->visaType->b2b_rate ?? 'N/A' }}
                                                    </td>
                                                    <td>
                                                        {{ $application->visaType->visa_fee ?? 'N/A' }}
                                                    </td>
                                                    <td>
                                                        @if ($application->status == 'approved')
                                                            <span class="badge bg-success">
                                                                Approved
                                                            </span>
                                                        @elseif($application->status == 'processing')
                                                            <span class="badge bg-warning">
                                                                Processing
                                                            </span>
                                                        @elseif($application->status == 'rejected')
                                                            <span class="badge bg-danger">
                                                                Rejected
                                                            </span>
                                                        @else
                                                            <span class="badge bg-secondary">
                                                                Pending
                                                            </span>
                                                        @endif
                                                    </td>
                                                    {{-- <td>
                                                        <div class="d-flex gap-2">
                                                            <a href="{{ route('visa-application.show', $application->id) }}"
                                                                class="btn btn-sm btn-outline-primary">
                                                                <i class="mdi mdi-eye"></i>
                                                            </a>
                                                            <a href="{{ route('visa-application.edit', $application->id) }}"
                                                                class="btn btn-sm btn-outline-success">
                                                                <i class="mdi mdi-pencil"></i>
                                                            </a>
                                                            <form
                                                                action="{{ route('visa-application.delete', $application->id) }}"
                                                                method="POST">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button class="btn btn-sm btn-outline-danger"
                                                                    onclick="return confirm('Are you sure?')">
                                                                    <i class="mdi mdi-delete"></i>
                                                                </button>
                                                            </form>
                                                        </div>
                                                    </td> --}}
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection
